...
// API implementation
/**********************************************************************************/
uint8_t g_CONTEXT_SWITCH_cont = 0;
...
static void dispatcher(task_switch_type_e type)
{
	...
	task_list.next_task = g_idle_task_index;
	uint8_t prioridad_mas_alta = -1;
	for(uint8_t i=0;i<task_list.nTasks;i++){//for cada tarea en lista de tareas do
		...
		if(task_list.tasks[i].priority >= prioridad_mas_alta && (task_list.tasks[i].state == S_READY || task_list.tasks[i].state == S_RUNNING)){
			...
			task_list.next_task = i;//siguiente_tarea = tarea
		}//end if
	}//end for
...
FORCE_INLINE static void context_switch(task_switch_type_e type)
{
	...
	if(0 == g_CONTEXT_SWITCH_cont) //Si es la primera vez aqui
	{
		if(kFromNormalExec == type)
		{ //Almacena el SP actual en el stack de la tarea actual
			task_list.tasks[task_list.current_task].sp = (uint32_t *)sp - STACK_FRAME_SIZE - 1;
		}
		else
		{
			task_list.tasks[task_list.current_task].sp = (uint32_t *)sp + STACK_FRAME_SIZE + 1;
